...
function ResetPasswordForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await api.resetAdminPassword(token, password);
      setDone(true);
    } catch (err) {
  ...
  if (done) {
    return (
      <AdminAuthPanel>
        ...
        <h2 style={{ fontFamily: "var(--font-heading)", fontSize: 26, margin: "18px 0 6px" }}>Password updated</h2>
  ...
  return (
    <AdminAuthPanel>
      <span className="page-eyebrow">Account recovery</span>
      <h2 style={{ fontFamily: "var(--font-heading)", fontSize: 26, margin: "18px 0 6px" }}>Choose a new password</h2>
      <p style={{ color: "var(--on-surface-variant)", marginBottom: 26 }}>Must be at least 8 characters.</p>
      <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <button className="primary" style={{ marginTop: 6 }} disabled={loading || mismatch || !password}>
