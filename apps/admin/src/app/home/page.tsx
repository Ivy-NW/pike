    ...
    <>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: 32 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", marginBottom: 20 }}>
          <div>
            <h1 style={{ fontFamily: "var(--font-heading)", margin: 0 }}>Platform oversight</h1>
            <p style={{ color: "var(--on-surface-variant)", marginTop: 4 }}>Businesses, venues, quests, and redemption review.</p>
          </div>
        ...
        {tab === "businesses" && (
          <>
            <div className="card">
              <h3 style={{ marginTop: 0, fontFamily: "var(--font-heading)" }}>Sales-assisted onboarding</h3>
          ...
          <>
            <div className="tab-bar" style={{ marginBottom: 12 }}>
              {(["", "claimed", "flagged", "rejected"] as const).map((s) => (
                <button key={s || "all"} className={redemptionFilter === s ? "active" : ""} onClick={() => setRedemptionFilter(s)}>
                  {s || "all"}
                </button>
              ))}
            </div>
