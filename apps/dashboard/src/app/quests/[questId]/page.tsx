...
export default function QuestDetailPage() {
  ...
  if (error) return <p style={{ padding: 32, color: "var(--error)" }}>{error}</p>;
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: 32 }}>
        ...
        {marker && (
          <div className="card" style={{ marginTop: 16 }}>
            <h3 style={{ marginTop: 0, fontFamily: "var(--font-heading)" }}>Marker</h3>
            <p>Status: {marker.status}</p>
            {marker.qrFallbackUrl && (
              <>
                <p style={{ fontSize: 14, color: "var(--on-surface-variant)" }}>Fallback QR (encodes the quest URL):</p>
                <img src={marker.qrFallbackUrl} alt="Quest QR code" width={160} height={160} />
              </>
            )}
            {marker.printAssetUrl && <p style={{ fontSize: 13, color: "var(--outline)" }}>Print asset: {marker.printAssetUrl}</p>}
          </div>
        )}
      </div>
    </>
