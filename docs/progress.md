...
| Push notifications — logic (FR-6) | Implemented | `NotificationsService`: device-token registration (`POST /users/me/push-token`, new `PushToken` model + migration `20260727110000_add_push_token`); two triggers — streak-expiry (daily `@Cron` selecting users who completed *yesterday* by UTC day) and new-quest-at-a-favorited-venue (fired from quest publish). Supports both web push (VAPID) and native Expo push |
...
| C | Backend wiring | **In Progress** (2026-08-21) | Deterministic custodial addresses, Redis mint queue, batch service implemented. Services created: `TokenWalletService` (HD derivation), `TokenQueueService` (Redis), `TokenChainService` (contract calls), `TokenBatchService` (scheduled batches). Wired to badge awards and reward claims. Missing: voucher burns, reconciliation sweep, tx hash persistence |
...
- ~~FR-6 largely built (favorited venues + notification triggers/registration, 2026-07-27). Remaining FR-6: real Expo/FCM delivery (swap the `NotificationsService.send` stub for a provider once `FCM_SERVER_KEY` + `expo-server-sdk`/`firebase-admin` exist) and the app's `expo-notifications` token acquisition on launch.~~ **FR-6 complete** (2026-08-21) — Expo push delivery via `expo-server-sdk`, web push via `web-push` with VAPID keys. App registers tokens on launch via `expo-notifications` + `expo-device`. All 42 API tests passing.
...
- Soulbound token layer designed (2026-07-31, [PRD](PIKE_token_layer_PRD.md) + [ADR 0007](adr/0007-mirror-badges-and-rewards-as-soulbound-tokens.md)). Phase A + B complete (contracts written, tested, deployed to Fuji). **Next: Phase C backend wiring** — deterministic custodial addresses, Redis mint queue, batch service, reconciliation sweep.
